    ...
    <div className="min-h-dvh bg-page">
      ...
      <main className="pt-32 pb-20">
        <Container className="max-w-3xl">
          ...
          <p className="text-sm text-muted mb-12">{t("privacy.lastUpdated")}</p>

          <p className="text-body-text leading-relaxed mb-10">{t("privacy.intro")}</p>

          {sections.map((key) => (
            <section key={key} className="mb-10">
              <h2 className="text-xl font-body-bold text-heading mb-3">
                {t(`privacy.${key}.title` as TranslationKey)}
              </h2>
              <p className="text-body-text leading-relaxed">
                {t(`privacy.${key}.desc` as TranslationKey)}
                {key === "contact" && (
                  <>
                    {" "}
                    <a
